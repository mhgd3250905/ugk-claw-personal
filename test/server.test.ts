...
test("GET /playground uses touch-first mobile panels for library, tasks, conn, and history", async () => {
	...
	assert.match(response.body, /@media \(max-width: 640px\) \{[\s\S]*\.asset-modal-head\s*\{[\s\S]*background:\s*transparent;/);
	...
	assert.match(response.body, /@media \(max-width: 640px\) \{[\s\S]*\.task-inbox-result-bubble\s*\{[\s\S]*min-height:\s*64px;[\s\S]*border-radius:\s*4px;/);
	assert.match(response.body, /@media \(max-width: 640px\) \{[\s\S]*\.conn-manager-toolbar\s*\{[\s\S]*grid-template-columns:\s*1fr;[\s\S]*background:\s*transparent;/);
	...
	assert.match(response.body, /@media \(max-width: 640px\) \{[\s\S]*\.task-inbox-head\s*\{[\s\S]*background:\s*transparent;/);

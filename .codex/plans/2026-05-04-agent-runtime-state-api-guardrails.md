...
**目标：** 防止 agent 自作主张直接编辑运行态底层文件 / SQLite / JSON index，统一要求通过项目已经设计好的 HTTP API、工具或服务层方法操作状态，避免磁盘状态、内存 registry、索引、通知和前端视图互相分裂。
...
- **凡是会维护多处状态的能力，禁止手写底层状态。** 包括内存 registry、SQLite 表、JSON index、运行目录、通知广播、缓存、会话文件等。
- **先软护栏，再硬护栏。** 第一轮先补技能 / 文档 / 测试，第二轮再补后端 reload / repair / reconcile 入口。
...
**目标：** 先让 agent “知道不能碰哪里”，并用测试防止规则被删松。
...
- agent profile、conn、asset、conversation、skill、model、Feishu、activity 这几类状态都有明确“禁止直接改底层”的规范。
...
- 不一次性重构所有存储层。
- 不禁止只读排障；禁止的是绕过 API 的写操作。
- 不把所有运行态文件改成只读文件系统；当前先通过规范、测试和官方接口治理。

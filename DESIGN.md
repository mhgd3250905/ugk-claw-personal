...
Chat content should follow the composer width. On desktop, use a two-column cockpit layout: a compact left conversation rail with the single `UGK CLAW` brand signal in its header, a transparent top command lane for actions only, and a large right work stage that contains the landing canvas, transcript, and composer. The desktop command lane should feel like a geek control panel, not a form toolbar; keep actions tight and visually subordinate to the work stage. Empty landing mode may center the command lane so the first viewport feels balanced; active conversation mode should keep the work rhythm stable. On mobile, prioritize the transcript and composer over explanatory chrome. The global mobile topbar is navigation, not a panel: keep the topbar, brand trigger, context slot, new-chat button, and overflow trigger transparent with no shadows; let the page background and actual drawers / menus provide depth. Controls should be icon-first when their meaning is familiar, with accessible labels preserved in markup. Fixed-format controls need stable dimensions so streaming status, hover states, and button labels do not shift the layout.
...
The desktop shell uses cockpit surfaces rather than marketing hero composition: the left rail is a narrow index surface with a small active accent, the right `chat-stage` is the primary work canvas, and the landing composer is a compact command deck pinned near the lower center of the stage. Empty landing mode may keep generous space for the wordmark, but active chat mode must tighten the transcript top inset so the first visible message starts close to the work canvas edge instead of floating under a hero-sized gap. Dark mode uses near-black solid layers with faint cool accents; light mode maps the same hierarchy to a transparent `chat-stage` over the cool workspace atmosphere, with white or cool-white surfaces reserved for real content carriers. Dark edge fades must never leak into the light theme.

The project ASCII mark is a real DOM `<pre>` brand asset, not a CSS `content` pseudo-element. Use the same `ugk-ascii-logo-topbar` variant for both the desktop conversation rail header and the mobile brand button; the mobile button may constrain the container size, but it must not switch to a separate shrunken artwork. Use the center `chat-stage` watermark as a lower-contrast variant so glyph layout is owned by one stable renderer. Keep the watermark non-interactive and low contrast: dark theme uses a faint cool white/blue tint, light theme uses a lower-opacity action blue tint. Render all ASCII variants with a box-drawing-capable system monospace such as Courier New / Consolas / Cascadia Mono instead of the app's Agave mono token; Agave does not reliably align block and box-drawing glyphs. Header variants may use subtle red / blue / amber chromatic offsets, but the center watermark must never reduce transcript readability or sit above message cards.

Transcript content should align to the full `#command-deck.command-deck` width, not to an inner composer child. The command deck is the visible bottom rail, so message bubbles, status rows, and error banners should share that left and right edge even when file chips or the textarea resize internally.

Assistant messages use dark raised surfaces. User messages use a clear opposing treatment but keep text left-aligned for readability. In light mode, user messages should read as compact right-aligned input echoes: white or cool-white surface, dark body text, and a soft green border that marks the user source without adding a vertical accent bar, not a recycled assistant reading card or a dark gray bubble left over from the dark theme. Error banners are opaque, high-contrast, and floating; they must not be rendered as semi-transparent overlays that rely on whatever happens to sit behind them.

The composer is one control surface, not a textarea floating inside a form. In light mode, the broad landing `command-deck` should stay transparent so it sits inside the workspace instead of becoming another white panel, while `#composer-drop-target.composer` keeps a cool-white fill as the real composer surface. `.file-strip` is only a layout container for selected files and reusable assets; it should stay transparent so it does not create a nested panel inside the composer zone. Visible containment belongs to the composer drop target, the message input itself, buttons, file chips, and focus outline. The `#message` textarea must keep a cool-white fill in light mode. When the message input or any composer control receives focus, put the active outline on `#composer-drop-target.composer:focus-within`; the inner textarea should keep `outline: none` and must not change to the accent border. This keeps focus visible without making the input feel like a generic backend form field.

Run log dialogs are audit surfaces, not transcript mirrors. Show newest meaningful entries first, load older entries in small pages as the user scrolls down, and omit raw body deltas. Each entry body must be capped to a readable preview so a single tool payload cannot dominate the panel. Light mode must use pale surfaces with dark text for the current chat run log and conn run detail panels; never reuse the dark-panel white text treatment on a light background.

Transcript message actions live inside the `.message-body` at the bottom edge of the rendered content, but only after that message has real content such as text, attachments, referenced assets, or files. Empty assistant active-run placeholders must not render the action rail, because the rail itself creates a visible empty body. Keep actions as compact icon-only affordances with accessible labels, no visible text, no border, no background, and no shadow. The action rail should sit on the message body's grid rhythm without adding its own top margin; assistant message body spacing stays compact so light bubbles do not grow a dead blank footer. The copy action and image export action share the same quiet control rail; PNG exports must omit the action rail itself and add a small `UGK Claw` signature outside the rendered message body. Message-image export clones must stay origin-clean: strip external style resources, replace embedded media with a compact placeholder, and load the `foreignObject` SVG intermediate as a `data:image/svg+xml` URL before drawing to canvas.

...
export function formatSearchContext(query: string, results: SearXNGResult[]): string {
	...
	const visible = results.filter((result) => result.url).slice(0, 10);
	if (visible.length === 0) {
	...
	visible.forEach((result, index) => {
		...
		lines.push(`${index + 1}. ${result.url} - ${result.title || "(untitled)"}${truncated ? ` - ${truncated}` : ""}`);
	});
	return lines.join("\n");
}
...
export async function searchAndFormat(queries: string[]): Promise<string> {
	...
	for (const query of queries) {
		try {
			...
			console.log(`[team] search "${query}" returned ${results.length} results`);
			parts.push(formatSearchContext(query, results));
		} catch (err) {
			const msg = (err as Error).message;
			console.log(`[team] search "${query}" failed: ${msg}`);

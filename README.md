...
> 需要 Node.js 22+、Docker、`ANTHROPIC_AUTH_TOKEN`。生产部署从 `.env.example` 复制为 `.env`，`docker compose -f docker-compose.prod.yml up --build -d`。
...
支持智谱 GLM、DeepSeek、小米（MiMo）三类模型源。在 Playground 的模型设置里切换。
</details>

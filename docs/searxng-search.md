...
用户侧只需要记住 `/searx:` 或 `/searxng:` 前缀，后面优先用自然语言描述搜索意图：

```text
/searx:国产大模型最新进展，最近一个月，给我 5 条中文结果
```
...
- `--time-range`：时间范围，可用 `day`、`month`、`year`。
- `--limit`：返回条数，默认 `10`。
- `--dry-run`：只看请求构造，不真正联网。

后续如果要让 Playground 稳定识别“最近一个月 / 给我 5 条 / 英文结果”这类自然语言参数，需要在技能规则或调用层明确解析，不能靠 agent 自由猜。否则用户以为自己限制了时间范围，实际走默认全量搜索，这种假聪明很烦。
...
- `http://127.0.0.1:3000/healthz` 返回 `{"ok":true}`。
- `ugk-pi-searxng` 容器已启动，宿主调试端口为 `127.0.0.1:48080`。
- 通过 `/searx:国产大模型 最新进展` 在 Playground 实测可用。
...
- SearXNG 不是自带索引库，而是从服务器出口访问 Google、DuckDuckGo、Startpage、Brave 等上游 engine。部署到阿里云后，如果服务器访问不了 Google 或某些海外搜索源，对应 engine 就会超时、403 或进入 `unresponsive_engines`；只要还有可访问 engine，仍可能返回结果。不要把“本机搜得到”直接等价成“阿里云一定搜得到”，这俩出口网络不是一回事。

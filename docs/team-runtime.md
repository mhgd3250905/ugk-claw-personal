...
| `src/team/team-submit-tools.ts` | submit tool 静态规格和 role/tool 到 stream 的映射；当前只定义契约，不代表真实 provider tool calling 已接通 |
...
- `team-submit-tools.ts` 是 submit tool 规格与映射表，`llm-tool-loop.ts` 是 provider-api-aware tool loop 底座。`TeamOrchestrator` 会在执行支持 submit tool 的 runner 时注入 task 级 handler；Discovery tool call 可以经 `submitTeamStreamItem()` 立即写入 stream。没有真实 role 或模型未触发 tool call 时仍保留 JSON envelope 兼容路径。
...
4. **真实 submit tool loop 先覆盖 Discovery tracer bullet** — `src/team/llm-tool-loop.ts` 已提供 submit tool loop 底座，`TeamOrchestrator` 已能把 Discovery tool call 写入 `candidate_domains` 并让后续 ready task 在同一 tick 看到新增 stream item；Evidence / Classifier / Reviewer 真实 tool submit 仍未逐一接通

...
5. `done.files` 返回给前端
6. playground 渲染文件卡片
7. canonical conversation history 也会把这些 `send_file` 结果挂回对应 assistant 消息，避免文件卡片在刷新或晚到的 state 回包后凭空消失
...
- agent 每轮执行现在都会生成独立的 browser cleanup scope，并在实际 `session.prompt(...)` 期间通过环境变量把该 scope 透给 `web-access`
- 正常完成、报错或中断后，`AgentService` 都会在 `finally` 里 best-effort 调用 `closeBrowserTargetsForScope(scope)`，清理本轮遗留页面

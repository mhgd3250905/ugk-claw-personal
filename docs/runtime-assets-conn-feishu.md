...
- 后台 run 的浏览器路由独立解析：优先使用 conn 自身 `browserId`，否则使用 resolved Agent snapshot 里的 `defaultBrowserId`，再否则显式使用 Browser Registry 的 `defaultBrowserId`。runner 会把最终 browserId 写入 browser scope route，并通过后台 session 的 Bash 工具注入 `CLAUDE_AGENT_ID` / `CLAUDE_HOOK_AGENT_ID` / `agent_id` 和 `WEB_ACCESS_BROWSER_ID`，所以 `web-access` 既能按 scope 找到正确 Chrome，也不会被长驻 proxy 的旧环境变量带跑。前台 chat session 和后台 conn session 还会在 run workspace 前置一个受控 `curl` wrapper：只要命令访问 `http://127.0.0.1:3456` 或 `http://localhost:3456`，wrapper 会自动补上本轮 `metaAgentScope` / `metaBrowserId`。这层的目标很朴素：给这次 run 换哪根 Chrome 管子，它通过常规 `curl` 就只能看见哪根管子，不要靠模型每次都记得手写参数。`web-access` 对带 `metaAgentScope` 的请求不会再使用 proxy 进程自己的 `WEB_ACCESS_BROWSER_ID` 兜底；没有命中 scope route 时会使用系统默认浏览器。不要把“没有选择浏览器”实现成空值；空值会让旧 proxy 自己用进程环境兜底，浏览器切换后就很容易串到上一次的 Chrome。

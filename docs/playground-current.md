...
- 手机端文件库不再按桌面居中弹窗压缩显示，而是底部抽屉式面板：面板贴底、最高约 `88dvh`、顶部有短 handle、标题区 sticky 且背景透明；标题区只保留标题与单行动作工具栏，不再显示说明句，也不再把动作按钮拆成两列网格，底部保留 `safe-area` 空间，避免拇指操作时被系统手势区吃掉
...
- 手机端任务消息页复用贴底抽屉语言：任务结果正文按对话气泡规格渲染，卡片结构是“消息元信息 / 结果气泡 / 底部动作”，正文走与 transcript 相同的 markdown hydration，文件结果复用下载卡片；点开“查看过程”后的 run detail `Result` 也按同一套消息正文规格渲染完整 `resultText`；任务结果 Markdown 正文字号收口为 `12px`，标题按 `18px / 16px / 14px` 分级，链接、代码、引用和表格头使用轻量颜色区分；列表卡片最小触摸高度为 `64px`。
...
- 这些手机面板的共同约束是：标题区 sticky、背景透明、动作入口优先收成单行横向工具栏，内容区独立滚动并 `overscroll-behavior: contain`，主操作按钮最小高度约 `40px`，列表项最小高度 `64px`，底部 padding 包含 `env(safe-area-inset-bottom)`。

...
function mergeRunHistoryItems(
  ...
  const byRunId = new Map(apiItems.map((item) => [item.run.runId, item]));
...
function readStoredTaskBranches(value: unknown): TaskBranchState[] {
  ...
  for (const item of value) {
    ...
    const detailMode: TaskBranchDetailMode | null =
      rawDetailMode === "leader-chat" || rawDetailMode === "edit" || rawDetailMode === "run-history" || rawDetailMode === "run-observer" || rawDetailMode === "discovery-subcanvas"
        ? rawDetailMode
        : null;
